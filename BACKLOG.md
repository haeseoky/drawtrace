...
- **현재:** 6종 게임 (따라그리기, 반응속도, 기억력카드, 컬러매치, 벽돌깨기, 2048)
- **완료 (2026-09-18):** 2048 추가 — 4x4 보드, 스와이프/방향키 조작, 병합 애니메이션, 리더보드/공유 연동, a11y(aria-live) 적용
...
- **완료 (2026-09-10):** 패들 최소 수직 속도 보장(기존) + 이동 정규화 단계 안티락 클램프 추가 — 벽돌 반사/카오스 왜곡 후에도 dx·dy 최소 18% 속도 보장
...
- **완료 (2026-09-12):** 정수 해시 → 문자열 키(충돌 제거), 9셀 miss 시 전체 탐색 fallback
- **기대효과:** 채점 정확도 향상, 점수 편차 감소

### 8. ✅ ColorMatchGame 초기 화면 개선
- **설명:** idle 상태에서 빈 화면 노출
- **완료 (2026-09-09):** 인트로 오버레이 추가 (게임명 + 규칙 안내)
- **기대효과:** 첫인상 개선, 플레이 유도

### 9. ✅ MemoryGame 점수 밸런스
- **설명:** 페널티가 보너스보다 커지면 0점 가능
- **완료 (2026-09-11):** 이동 패널티를 획득 보너스의 최대 50%로 상한 설정 — 라운드별 최소 보상 보장

### 10. ✅ vite 코드 스플리팅 완성
- **설명:** 일부 게임만 manualChunks로 분리됨
- **방법:** 5개 게임 모두 별도 청크로 분리
- **현재:** BrickBreakerGame, DrawGame, scorer만 분리, 나머지 3개는 메인 번들
- **완료 (2026-09-12):** ColorMatch/Memory/Reaction도 별도 청크 분리 — 5개 게임 전체 분리 검증
...
- **완료:** visibilitychange로 탭 복귀 시 wait 재시작 처리 이미 적용됨 (2026-09-11 코드 확인)

### 13. ✅ DrawGame 이중 드로우 최적화
- **완료:** onTouchStart에서 첫 프레임만 drawFrame() 직접 호출로 이미 적용됨 (2026-09-11 코드 확인)

### 14. ✅ share.js BASE_URL 하드코딩 제거
- **완료:** `VITE_BASE_URL || 'https://game.nutalk.co.kr'` 폴백으로 이미 적용됨 (2026-09-11 코드 확인)

### 15. ✅ 라우터 404 처리
- **완료:** catch-all 리다이렉트 라우트 이미 존재 (2026-09-11 코드 확인)

### 16. 🔧 접근성(a11y) 강화
- **설명:** 스크린 리더, 키보드 플레이 지원
- **방법:** 게임별 aria-live 영역, 키보드 컨트롤 매핑
- **현재:** 기본 aria-label만 적용
- **완료 (2026-09-17):** 5개 게임 aria-live 발표 + BrickBreaker 키보드 컨트롤(←/→ 이동, Space/Enter 시작·발사, R 재시작). 역방향 컨트롤 아이템도 키보드에 자동 반영
- **기대효과:** 웹 접근성 준수
